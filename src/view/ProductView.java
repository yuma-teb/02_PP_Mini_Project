...
public class ProductView {
  ...
  private void handlePagination(String choice) {
    switch (choice) {
      ...
      case "g":
        do {
          int gotoPage = Integer.parseInt(Helper.getAndValidate("Enter page number: ", "Page Number cannot be empty",
              "-?\\d+", "Wrong input format. Please input a number"));
          if (gotoPage < 0) {
  ...
  private void setRow() {
    do {
      int row = Integer.parseInt(Helper.getAndValidate("Please input number of row per page : ",
          "Row number cannot be empty", "-?\\d+", "Invalid input. Please Input a number!"));
      if (row <= 0) {
  ...
  private void update() {
    int id = Integer.parseInt(Helper.getAndValidate("Input ID to update: ", "Input ID cannot be empty", "^\\d+$",
        "Id must be input as number", 3));
  ...
  // backup (option ba)
  private void backup() {
    String confimation = Helper.getAndValidate("Are you sure you want to backup the data (y/n)? : ",
        "You cannot input empty value", "[ynYN]", "Please input y or n");
  ...
  private void restore() {
    ...
    if (backUpFileName.size() <= 0) {
      ...
      restore: do {
        ...
        if (breaker == 3) {
          String confirmation = Helper.getAndValidate("Do you want to continues (y/n)? : ", "Choice cannot be empty",
              "[ynYN]", "Invalid input please input y or n");
